...
__global__ void calculate_sum(const MultivariateCoordinate *coord, double *out,
                              Operation op) {
  int idx = threadIdx.x;
  double sum = 0;
...
__global__ void calculate_sum(const MultivariateCoordinate *coord, double *out,
                              double mean1, double mean2, Operation op) {
  int idx = threadIdx.x;
  double sum = 0;
...
std::vector<MultivariateCoordinate> read_mock_data(const char *filepath) {
  ...
  in.read_header(io::ignore_extra_column, "name", "points", "skill", "assists",
                 "salary");
...
double SumWithCuda(const std::vector<MultivariateCoordinate> &mc,
                   Operation op) {
  // Variables on CPU
  ...
  cudaStatus =
      cudaMalloc((void **)&dev_coord, n * sizeof(MultivariateCoordinate));
  if (cudaStatus != cudaSuccess) {
  ...
  cudaStatus = cudaMemcpy(dev_coord, coord, n * sizeof(MultivariateCoordinate),
...
double SumWithCuda(const std::vector<MultivariateCoordinate> &mc, double mean,
                   Operation op) {
  // Variables on CPU
  const MultivariateCoordinate *coord = &mc[0];
  double out[ARR_SIZE];
...
double SumWithCuda(const std::vector<MultivariateCoordinate> &mc, double mean1,
                   double mean2, Operation op) {
  // Variables on CPU
  const MultivariateCoordinate *coord = &mc[0];
  double out[ARR_SIZE];
  int n = mc.size();
...
CalculateGradientAndYIntercept(const std::vector<MultivariateCoordinate> &mc) {
  ...
  double b1 = (sum_of_products_x1_y * sum_of_squares_x2 -
               sum_of_products_x1_x2 * sum_of_products_x2_y) /
              (sum_of_squares_x1 * sum_of_squares_x2 -
               sum_of_products_x1_x2 * sum_of_products_x1_x2);

  double b2 = (sum_of_products_x2_y * sum_of_squares_x1 -
               sum_of_products_x1_x2 * sum_of_products_x1_y) /
              (sum_of_squares_x1 * sum_of_squares_x2 -
               sum_of_products_x1_x2 * sum_of_products_x1_x2);
